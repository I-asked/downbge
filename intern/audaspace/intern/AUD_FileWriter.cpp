...
std::shared_ptr<AUD_IWriter> AUD_FileWriter::createWriter(std::string filename,AUD_DeviceSpecs specs,
														AUD_Container format, AUD_Codec codec, unsigned int bitrate)
{
#ifdef WITH_SNDFILE
	try
	{
		return std::shared_ptr<AUD_IWriter>(new AUD_SndFileWriter(filename, specs, format, codec, bitrate));
	}
	catch(AUD_Exception&) {}
#endif
	...
	try
	{
		return std::shared_ptr<AUD_IWriter>(new AUD_FFMPEGWriter(filename, specs, format, codec, bitrate));
	}
	catch(AUD_Exception&) {}
#endif
...
void AUD_FileWriter::writeReader(std::shared_ptr<AUD_IReader> reader, std::shared_ptr<AUD_IWriter> writer, unsigned int length, unsigned int buffersize)
{
...
void AUD_FileWriter::writeReader(std::shared_ptr<AUD_IReader> reader, std::vector<std::shared_ptr<AUD_IWriter> >& writers, unsigned int length, unsigned int buffersize)
{
